...
public class AKPickerView: UIView, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, AKCollectionViewLayoutDelegate {
	...
	*/
	private func selectItem(item: Int, animated: Bool, notifySelection: Bool) {
        ...
        let lastSelect = self.selectedItem
        ...
        if let lastCell = self.collectionView.cellForItemAtIndexPath(NSIndexPath(forItem: lastSelect, inSection: 0)) as? AKCollectionViewCell {
            lastCell._selected = false
        }
        
        if let selectCell = self.collectionView.cellForItemAtIndexPath(NSIndexPath(forItem: item, inSection: 0)) as? AKCollectionViewCell {
            selectCell._selected = true
        }
